...
本目录发行 **`seed`**（Python 包 `seed`，内含 **`seed.core`** 与 **`seed.integrations`**）。内置 LLM 可调工具在 **`seed-tools`** 发行版（导入前缀 **`seed_tools`**）。

架构与依赖方向见 [docs/PACKAGE_LAYOUT.md](docs/PACKAGE_LAYOUT.md)（**Seed 不依赖 CodeAgent 等产品包；宿主只依赖 Seed**）；模块级公开 API 见仓库根的 [PUBLIC_API.md](../PUBLIC_API.md)。

宿主集成路径与 **`SEED_*` / `CODEAGENT_*` 环境变量**统一说明见 [docs/INTEGRATION.md](docs/INTEGRATION.md) 与 [docs/ENV_REFERENCE.md](docs/ENV_REFERENCE.md)。
...
参考本仓库中 CodeAgent 的配置：[codeagent/pyproject.toml](../codeagent/pyproject.toml) 内 `tool.pytest.ini_options.pythonpath`（`../seed` 与 `../seed-tools` 指向 monorepo 下的两个发行根）。
...
二者在 **`seed.integrations`**：`seed.integrations.cron_sched`、`seed.integrations.env_config`（读取 `config/seed.env`，若无则回退 `config/codeagent.env`），不在 `seed.core`。
